...
const WalletView = (() => {
  ...
  async function render(el) {
    ...
    // Await the subscription read up front. Without this, a fresh load
    // on /security?tab=wallet renders before Subscription.init has had
    // a chance to populate its cache — so isPro() returns false and
    // every add-on card shows "Pro required" despite the user being
    // on Pro in the DB.
