...
const BlueprintsView = (() => {
  ...
  // Custom dropdown helpers live in the shared CSelect lib module
  // (app/js/lib/cselect.js). Keep these thin local aliases so existing
  // call sites read the same and future call sites can use the lib name.
  const _cselectHTML = CSelect.html;
  const _mountCselect = CSelect.mount;
  const _setCselect = CSelect.set;
