/* ═══════════════════════════════════════════════════════════════════
   NICE — Documentation Content (SSOT)

   Single source of truth for documentation content. Consumed by:
     • app/js/views/docs.js — renders the in-app Documentation hub
     • scripts/build-docs.mjs — generates www/docs/*.html marketing pages

   Edit content here, then run `npm run build:docs` to regenerate the
   marketing pages. The app picks up changes on next page load — no
   build step needed for the in-app surface.

   Content uses plain semantic HTML (<h1>, <h2>, <p>, <ul>, <table>…)
...
(function (root, factory) {
  ...
  /* ── Section metadata ─────────────────────────────────────────
     ...
     `marketingIcon` — Font Awesome class for the marketing site
     `description`   — SEO meta + landing-page card body
  ─────────────────────────────────────────────────────────────── */
  const SECTIONS = [
    {
      id: 'getting-started',
    ...
    },
    {
      ...
      label: 'Agents',
      icon: 'bot',
      marketingIcon: 'fa-robot',
      description: 'AI workers with specialized skills. Activate from the catalog or build your own in the Workshop.',
    },
    {
      id: 'spaceships',
      slug: 'spaceships',
    ...
    },
    {
      ...
      label: 'Tokens & XP',
      icon: 'zap',
      marketingIcon: 'fa-bolt',
      description: 'Subscription tiers, token pools, top-up packs, and the rank progression system.',
    },
    {
      id: 'keyboard',
      slug: 'keyboard',
      label: 'Keyboard',
      icon: 'terminal',
      marketingIcon: 'fa-keyboard',
      description: 'Modifier shortcuts and chord ladders for power-user navigation.',
    },
  ];

  /* ── Section bodies ───────────────────────────────────────────
     Plain semantic HTML. Both surfaces scope typography under
     `.docs-content` so headings, paragraphs, lists, and tables
  ...
  const BODIES = {
    ...
    /* The 'models' section has dynamic content in-app: docs.js replaces
       ...
       when rendering. The static rows here serve the marketing site and
       any context where the catalog isn't available. The marker
    ...
    'models': `
      ...
      <table data-models-table>
        ...
        <tbody>
          ...
          <tr><td>GPT-OSS 120B</td><td>OpenAI</td><td><span class="docs-badge docs-badge-pro">Premium</span></td><td>Open-weights frontier reasoning on Groq silicon</td></tr>
          <tr><td>Grok 4.3</td><td>xAI</td><td><span class="docs-badge docs-badge-pro">Premium</span></td><td>1M context, always-on reasoning, real-time research</td></tr>
    ...
    'keyboard': `
      ...
      <h2>Modifier Shortcuts</h2>
      <table>
        <thead><tr><th>Shortcut</th><th>Action</th></tr></thead>
        <tbody>
          <tr><td><kbd>Cmd/Ctrl + K</kbd></td><td>Command Palette</td></tr>
          <tr><td><kbd>Cmd/Ctrl + Shift + O</kbd></td><td>New Chat</td></tr>
          <tr><td><kbd>Cmd/Ctrl + Shift + P</kbd></td><td>Toggle Preview Panel</td></tr>
          <tr><td><kbd>?</kbd></td><td>Show Shortcuts overlay</td></tr>
        </tbody>
      </table>

      <h2>Navigation Chords</h2>
      ...
      <table>
        ...
        <tbody>
          ...
          <tr><td><kbd>G  V</kbd></td><td>Go Vault</td></tr>
          <tr><td><kbd>G  P</kbd></td><td>Go Profile</td></tr>
          <tr><td><kbd>G  T</kbd></td><td>Go Settings</td></tr>
          <tr><td><kbd>G  L</kbd></td><td>Go Captain's Log</td></tr>
          <tr><td><kbd>N  A</kbd></td><td>New Agent</td></tr>
        </tbody>
      </table>
