...
> Test coverage for untested load-bearing logic + defensive hardening on the recurring defect classes. Started 2026-06-17. **The named Phase 1 list is complete** (days 1+2); only the carve-out billing-race item and an optional listener-sweep lead remain.
...
- `[READY]` (lead) Listener-leak sweep of the *other* views — `spaceships.js:555`, `prompt-panel.js:2989/3015/3041` add `document` listeners. `schematic.js:575-576` was verified clean (`menu._cleanup` tears them down). The rest use *named* handlers (cleanup-intended); **verify each is per-render AND lacks matching removal before treating as a leak** — most are once-at-boot per the day-1 lesson, so this may close with no PR.
