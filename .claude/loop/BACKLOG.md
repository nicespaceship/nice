...
- **Day 3 (merged, self-merged per Q3):** coverage extended beyond the named list onto untested load-bearing logic — PromptPanel model-capability gate ([#865](https://github.com/nicespaceship/nice/pull/865)), SettingsView account-deletion gate ([#866](https://github.com/nicespaceship/nice/pull/866)), ShipBehaviors token-budget gate ([#867](https://github.com/nicespaceship/nice/pull/867)), Roles dispatch SSOT ([#868](https://github.com/nicespaceship/nice/pull/868)), AgentMemory ([#869](https://github.com/nicespaceship/nice/pull/869)). Suite 1913→1986.
- **Day 4 (merged, self-merged per Q3):** drained the rest of the no-steer coverage — RateLimiter ([#873](https://github.com/nicespaceship/nice/pull/873)), ModelCatalog + SSOT join ([#874](https://github.com/nicespaceship/nice/pull/874)), AgentBuilderView + SpaceshipBuilderView blueprint serialization via test seams ([#875](https://github.com/nicespaceship/nice/pull/875)/[#876](https://github.com/nicespaceship/nice/pull/876)), Favorites ([#877](https://github.com/nicespaceship/nice/pull/877)). Suite 2001→2096. **Clean high-value coverage vein now fully drained** — what's left is genuine low-value filler (message-bar / quick-notes) or DOM-heavy view-render tests; the next real pre-launch value (catalog growth) needs Ben's steer on verticals/tiers/count.
...
- 2026-06-19 — **Phase 1 coverage batch (5 PRs, all merged, self-merged per Q3):** five untested load-bearing modules pinned — PromptPanel model-capability/soft-fallback gate (`_modelHasCapability`/`_modelSatisfies`, [#865](https://github.com/nicespaceship/nice/pull/865)); SettingsView account-deletion gate (type-to-confirm + submit teardown/error paths, [#866](https://github.com/nicespaceship/nice/pull/866)); ShipBehaviors daily token-budget gate (checkBudget/deductBudget + midnight reset via fake timers, [#867](https://github.com/nicespaceship/nice/pull/867)); Roles dispatch SSOT (SEED getters + init() DB-load with all 3 SEED fallbacks, [#868](https://github.com/nicespaceship/nice/pull/868)); AgentMemory (FIFO trims, deep-clone isolation, buildPromptContext budget/ordering/recency, learn regex extraction, [#869](https://github.com/nicespaceship/nice/pull/869)). All build-checker PASS; suite 1913→1986. Each was the highest-bug-surface untested path on its surface; the clean high-value vein is now largely drained.
