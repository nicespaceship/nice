...
|--------|---------|-------|
| `mission_runs` table | `tasks` table | Rename pending |
| `missions.spaceship_id` NOT NULL | `missions.captain_id` nullable | Rename + enforce NOT NULL pending |
| WorkflowEngine absorbs MissionRouter patterns | `MissionRouter` is a parallel entry point | Fold-in pending |
| `missions.schedule` via pg_cron | `MissionScheduler` localStorage | Server-side scheduler pending |
| `workflow_runs` dropped | Legacy table exists | Drop pending |
| `missions.outcome_spec` / `tools_required` | Stored, never surfaced | Drop until a UI reads them |
| MissionRunner has no ephemeral-blueprint path | Lines 78-108 create agents on the fly | Remove pending |
