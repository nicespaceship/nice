...
### Database Tables
33 tables (`SELECT table_name FROM information_schema.tables WHERE table_schema='public'`). The list drifts — verify before assuming a table does or doesn't exist.
| Table | Purpose |
|-------|---------|
| `blueprints` | Agent + spaceship blueprints. `scope='catalog'` = seeded library (533), `scope='community'` = user-published/marketplace content |
...
| `fuel_usage` | Per-LLM-call usage metering: `model`, input/output tokens, `fuel_cost`, agent + spaceship |
| `fuel_purchases` | Stripe fuel-credit purchases: `amount` (USD), `fuel_amount`, `stripe_session_id` |
| `fuel_transactions` | Fuel ledger: `type`, `amount`, `balance_after` |
...
> **`fuel_*` vs `token_*`:** both credit systems coexist in-DB. The live views (`cost.js`, `analytics.js`) and `profiles.fuel_balance` use **fuel**; `token_balances`/`token_transactions` + the "Token Credit System" section below still describe the older **token** model. These need reconciling — treat the Token Credit System section as possibly stale until then.
...
- **Sub-floor micro chrome:** ✅ resolved by adding a micro tier to the scale: `--text-2xs` (10px) + `--text-3xs` (8px) for dense chrome (stat labels, badges, role/scope/type chips, timestamps, overlines). Tokens defined in [#784](https://github.com/nicespaceship/nice/pull/784); 101 values swept onto the tier in [#785](https://github.com/nicespaceship/nice/pull/785). Labels below the 8px floor stay raw (schematic `.sch-slot-*` / `.schematic-*` and `.ship-slot-max`); glyph/icon sizes and form-control inputs (iOS-zoom 16px guard) stay raw by design.
- **Marketing typography:** ✅ `public/css/theme.css` eyebrow tracking → 3-tier scale ([#786](https://github.com/nicespaceship/nice/pull/786)) + font-size → `--text-*` scale ([#787](https://github.com/nicespaceship/nice/pull/787)); `www/css/site.css` finished ([#788](https://github.com/nicespaceship/nice/pull/788)); `www/` inline `<style>` blocks ([#789](https://github.com/nicespaceship/nice/pull/789)); `--tracking-wordmark` (0.4em) introduced and applied to the live NICE SPACESHIP lockups. All app + marketing CSS typography is now tokenized.
- **Brand guide:** ✅ `www/brand.html` reconciled to the canonical scale ([#791](https://github.com/nicespaceship/nice/pull/791)) — it had forked `--text-*` larger (H2 48px vs the real 32px) and documented the fork in its labels; reset to canonical, relabeled, tokenized, wordmark mockups collapsed to `--tracking-wordmark`.
- **Typography epic COMPLETE** (#195–#206, #780–#791): app shell, both marketing stylesheets (`theme.css` + `site.css`), `www/` inline styles, and the brand guide are fully tokenized — prose + card + micro + 3-tier eyebrow tracking + font-size + wordmark. Any new raw `font-size`/`font-weight`/`letter-spacing`/`text-transform` is a bug.
