...
### Database Tables
34 tables (`SELECT table_name FROM information_schema.tables WHERE table_schema='public'`). The list drifts — verify before assuming a table does or doesn't exist.
| Table | Purpose |
|-------|---------|
...
| `workflows` | ⚠️ Exists in-DB but the Mission Ontology below says workflows-as-a-primitive was dropped; not read by client JS. Legacy — verify before relying on it. |
