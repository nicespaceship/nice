...
### Database Tables
31 tables (`SELECT table_name FROM information_schema.tables WHERE table_schema='public'`). The list drifts — verify before assuming a table does or doesn't exist.
| Table | Purpose |
|-------|---------|
...
> **`fuel_*` vs `token_*` (reconciled 2026-06-10):** there is ONE credit system — the **token** model (`token_balances` + `token_transactions`), described accurately in the "Token Credit System" section below. `fuel_usage` is NOT a credit system; it is per-call usage **telemetry** that feeds the Cost Tracker + Operations analytics. It went unwritten for months (so those views rendered empty); `nice-ai` now writes one row per LLM call. The dead credit-imposters `fuel_purchases`, `fuel_transactions`, and `profiles.fuel_balance` were dropped in `20260730000000_reconcile_fuel_token_credit_model.sql`.
