...
- **Next:** brand wordmark/badge pass — `www/brand.html` (self-contained showcase, no stylesheet link, 27 raw letter-spacings + display/eyebrow/wordmark tracking). Needs its own token-access approach (inline `:root` vs link `site.css`); may carry intentional demo tracking, so confirm before collapsing.
