...
| 25 | 2026-04-17 | C3 ops | Provider-level branch for OpenAI's newer-generation parameter rules. `openai` provider now sends `max_completion_tokens` (not `max_tokens`) and omits `temperature` (GPT-5+ / o-series require temperature=1). Mistral / DeepSeek / xAI keep `max_tokens` + explicit `temperature` since they still follow the classic Chat Completions contract. **Current production.** |
