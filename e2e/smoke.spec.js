...
test.describe('NICE Smoke Tests', () => {
  ...
  test('app loads and shows home view', async ({ page }) => {
    ...
    await expect(page.locator('.bridge-wrap')).toBeVisible();
  });
  ...
  test('all main views render without error', async ({ page }) => {
    const views = [
      { hash: '#/', title: 'Bridge' },
      { hash: '#/blueprints', title: 'Blueprints' },
      { hash: '#/blueprints/agents', title: 'Blueprints' },
      { hash: '#/blueprints/spaceships', title: 'Blueprints' },
      { hash: '#/integrations', title: 'Blueprints' },
  ...
  test('log view renders', async ({ page }) => {
    await page.goto('./');
    await expect(page.locator('#app-page-title')).toHaveText('Bridge', { timeout: 10000 });
    ...
    await expect(page.locator('#app-page-title')).toHaveText('Log', { timeout: 5000 });
    await expect(page.locator('.log-view-wrap')).toBeVisible({ timeout: 5000 });

    // Dock route redirects to Bridge
  ...
  test('bridge view has crew panel and mission feed', async ({ page }) => {
    await page.goto('./');
    await expect(page.locator('#app-page-title')).toHaveText('Bridge', { timeout: 10000 });

    // Bridge stats strip
    const stats = page.locator('.bridge-stats');
    await expect(stats).toBeVisible({ timeout: 5000 });

    // Bridge ship hero
    const hero = page.locator('.bridge-hero');
    await expect(hero).toBeVisible({ timeout: 5000 });

    // Bridge mission feed
    const feed = page.locator('.bridge-feed');
    await expect(feed).toBeVisible({ timeout: 5000 });
  });

  test('prompt panel is visible on bridge', async ({ page }) => {
    await page.goto('./');
    await expect(page.locator('#app-page-title')).toHaveText('Bridge', { timeout: 10000 });

    // Prompt panel should be present on Bridge (route-aware)
    const promptPanel = page.locator('#nice-ai');
    await expect(promptPanel).toBeVisible({ timeout: 5000 });

    // Text input should exist
    const input = page.locator('#nice-ai-input');
...
test.describe('NICE Accessibility', () => {
  ...
  test('blueprints view has no critical accessibility violations', async ({ page }) => {
    await page.goto('./');
    await expect(page.locator('#app-page-title')).toHaveText('Bridge', { timeout: 10000 });
    await page.evaluate(() => { window.location.hash = '#/blueprints'; });
    await expect(page.locator('#app-page-title')).toHaveText('Blueprints', { timeout: 5000 });
...
test.describe('NICE Feature Tests', () => {
  ...
  test('missions board view has kanban columns via log view', async ({ page }) => {
    await page.goto('./');
    await expect(page.locator('#app-page-title')).toHaveText('Bridge', { timeout: 10000 });
    ...
    // Missions tab should be active by default — switch to board view
    await page.locator('#btn-view-board').click({ force: true });
    await expect(page.locator('.mb-col').first()).toBeVisible({ timeout: 5000 });
    const columns = page.locator('.mb-col');
    const count = await columns.count();
    expect(count).toBe(4);
  });

  test('captain log view filters by category', async ({ page }) => {
    await page.goto('./');
    await expect(page.locator('#app-page-title')).toHaveText('Bridge', { timeout: 10000 });
    ...
    // Navigate around to generate some audit log entries
    await page.evaluate(() => { window.location.hash = '#/blueprints'; });
    await expect(page.locator('#app-page-title')).toHaveText('Blueprints', { timeout: 5000 });

    // Navigate to Log → Log tab
    await page.evaluate(() => { window.location.hash = '#/log?tab=log'; });
    await expect(page.locator('#app-page-title')).toHaveText('Log', { timeout: 5000 });

    // Should have log entries (from our navigation)
    const entries = page.locator('.audit-entry, .log-entry, .timeline-item');
  ...
  test('full journey: navigate zones and verify no errors', async ({ page }) => {
    ...
    // 3. Navigate to Dock
    // 3. Dock redirects to Bridge
    await page.evaluate(() => { window.location.hash = '#/dock'; });
    await expect(page.locator('#app-page-title')).toHaveText('Bridge', { timeout: 5000 });
    const bridgeStats = page.locator('.bridge-stats');
    await expect(bridgeStats).toBeVisible({ timeout: 5000 });

    // 5. Verify MissionRunner module is loaded
    ...
    // 6. Verify ShipLog module is loaded
  ...
  test('old routes redirect correctly', async ({ page }) => {
    ...
    // /analytics → /log?tab=operations
    await page.evaluate(() => { window.location.hash = '#/analytics'; });
    await page.waitForTimeout(500);
    expect(await page.evaluate(() => location.hash)).toContain('#/log');

    // /agents → /blueprints/agents
    await page.evaluate(() => { window.location.hash = '#/agents'; });
    await page.waitForTimeout(500);
    expect(await page.evaluate(() => location.hash)).toBe('#/blueprints/agents');
  });
  ...
  test('streamlined journey: prompt panel and agent matching', async ({ page }) => {
    ...
    // 1. Verify prompt panel is visible on Bridge
    const promptInput = page.locator('textarea.nice-ai-input');
    await expect(promptInput).toBeVisible({ timeout: 5000 });

    // 2. Test PromptPanel module is loaded
    ...
    // 5. Test agent keyword matching via State agents
...
test.describe('NICE Auth Flow', () => {
  ...
  test('unauthenticated users can navigate views without errors', async ({ page }) => {
    ...
    // Navigate to blueprints without authentication
    await page.evaluate(() => { window.location.hash = '#/blueprints'; });
    await expect(page.locator('#app-page-title')).toHaveText('Blueprints', { timeout: 5000 });
...
test.describe('NICE Performance', () => {
  ...
  test('view transitions complete within 1 second', async ({ page }) => {
    ...
    const views = ['#/blueprints', '#/log', '#/settings', '#/'];
  ...
  test('no memory leaks from rapid navigation', async ({ page }) => {
    ...
    const routes = ['#/', '#/blueprints', '#/log', '#/settings',
                     '#/', '#/blueprints', '#/log', '#/settings',
                     '#/', '#/blueprints', '#/log', '#/settings',
                     '#/', '#/blueprints', '#/log', '#/dock', '#/'];
